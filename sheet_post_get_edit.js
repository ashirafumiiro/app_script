...
function rowEdited(e){
  ...
  var watchColumns = [9,20,21,23,18]; //18 - stripe enabled
  ...
  const id = result._id;
  const {enabled, stripe_customer_id, stripe_subscription_id, stripe_status, stripe_account_id, stripe_price_id, comission, subscripiton_expiry, tax } = result;
  
  const payload = {
                    enabled: Boolean(enabled), 
                    // stripe_status: stripe_status.replaceAll('"', '') || "disabled", 
                    //stripe_account_id,
                    stripe_price_id, 
                    comission, 
                    tax
  ...
  var options = {
    'method' : 'patch',
    'headers': {
      "apikey": "my_super_secret_apikey"
    },
    'payload' : payload
  };
  var add = 'https://527d-154-0-128-42.eu.ngrok.io';
